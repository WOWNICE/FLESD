...
if __name__ == '__main__':
    ...
    processes = [
        Process(target=local_update_weights_infer_on_public,
                args=(gpu+1, args, local_models[gpu], train_dataset, inferset, pubidx, 1, user_groups[str(idx)], return_dict))
    ...
    # make sure it's the ensemble.
    assert(len(reps) > 1)
    
    # update global weights via knowledge distillation 
    # global_weights = global_update_ERD(0, args, model=copy.deepcopy(global_model), pubset=pubset, pubidxs=pubidx, reps=reps)
    manager = Manager()
    ...
    global_weights = global_return_dict[0][0]
    # get the representation norm.
    ...
    global_path = os.path.join(save_path, f"global.pth.tar")
